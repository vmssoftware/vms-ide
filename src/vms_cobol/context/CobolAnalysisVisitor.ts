...
const must_start_a = localize("must_be_in_a", "Must start in the A area");
...
export class CobolAnalysisVisitor extends AbstractParseTreeVisitor<void> implements cobolVisitor<void> {
    ...
    visitIdentification_division_header(ctx: Identification_division_headerContext) {
        if (ctx.IDENTIFICATION().symbol.charPositionInLine >= 4) {
            this.helper.mark(ctx.IDENTIFICATION(), must_start_a);
        }
        //this.visitChildren(ctx);
    }
    ...
    visitEnd_program_header(ctx: End_program_headerContext) {
        if (ctx.END().symbol.charPositionInLine >= 4) {
            this.helper.mark(ctx.END(), must_start_a);
        }
        //this.visitChildren(ctx);
    }
    ...
    visitDeclaratives_header(ctx: Declaratives_headerContext) {
        if (ctx.DECLARATIVES().symbol.charPositionInLine >= 4) {
            this.helper.mark(ctx.DECLARATIVES(), must_start_a);
        }
        //this.visitChildren(ctx);
    }
    ...
    visitParagraph(ctx: ParagraphContext) {
        ...
        if (nameCtx && nameCtx.symbol.charPositionInLine >= 4) {
            this.helper.mark(ctx.paragraph_name(), must_start_a);
        }
        this.visitChildren(ctx);
    }
    ...
    visitSection_header(ctx: Section_headerContext) {
        ...
        if (nameCtx && nameCtx.symbol.charPositionInLine >= 4) {
            this.helper.mark(ctx.section_name(), must_start_a);
        }
        this.visitChildren(ctx);
    }
    ...
    visitProgram_id(ctx: Program_idContext) {
        ...
        if (programCtx !== undefined && programCtx.parent instanceof ProgramContext) {
            ...
            if (common_initial !== undefined) {
                ...
                if (common !== undefined) {
                    this.helper.mark(common, localize("must_in_contained_program", "Can be used only in a contained program."));
                }
            }
        }
        //this.visitChildren(ctx);
    ...
    visitEnd_program(ctx: End_programContext) {
        ...
        if (symbol instanceof ProgramSymbol) {
            if (!symbol.context) {
                this.helper.mark(ctx.program_name(), localize("must.link.here", "Must link into this source file."));
            } else {
                // test if
    ...
    visitSpecial_names(ctx: Special_namesContext) {
        ...
        if ((contentCtx === undefined || contentCtx.text.length === 0) && ctx.DOT_().length > 1) {
            this.helper.mark(ctx.DOT_(1), localize("dot_must_end_some", "Must be at the end of a nonempty statement."))
        }
        this.visitChildren(ctx);
    }
    ...
    visitSymbol_char(ctx: Symbol_charContext) {
        ...
        if (symbCharSymbol instanceof SYMBOLIC_CHARACTERS_Symbol) {
            if (symbCharSymbol.resolve(symbCharSymbol.name) !== symbCharSymbol) {
                this.helper.mark(ctx, localize("isnt_unique", "Is not unique."));
            }
        }
        //this.visitChildren(ctx);
    }
    ...
    visitCurrency_char(ctx: Currency_charContext) {
        ...
        if (text && text.match(this.helper.rgxReservedPic)) {
            this.helper.mark(ctx, localize("invalid_currency", "Invalid currency symbol."));
        }
        //this.visitChildren(ctx);
    }
    ...
    visitStatement(ctx: StatementContext) {
        ...
        if (childInA) {
            this.helper.mark(childInA, localize("must_be_in_B", "Must start in the B area."));
        }
        this.visitChildren(ctx);
    }
        ...
        if (fileSymbol instanceof FileSymbol) {
            ...
            for(let fd_clause of ctx.fd_clause()) {
                if (fd_clause.linage() && fileSymbol.fileFormat !== EFileFormat.Sequentional) {
                    this.helper.mark(fd_clause, localize("must.be.sequentional", "Must be a sequential file."));
                }
                if (fd_clause.code_set() && !(fileSymbol.fileFormat === EFileFormat.Sequentional || fileSymbol.fileFormat === EFileFormat.Report)) {
                    this.helper.mark(fd_clause, localize("must.be.seq.or.rep", "Must be a sequential or report file."));
                }
                if (fd_clause.record_key() && fileSymbol.fileFormat !== EFileFormat.Indexed) {
                    this.helper.mark(fd_clause, localize("must.be.indexed", "Must be an indexed file."));
                }
                if (fd_clause.alt_record_key() && fileSymbol.fileFormat !== EFileFormat.Indexed) {
                    this.helper.mark(fd_clause, localize("must.be.indexed", "Must be an indexed file."));
                }
                let dup_str = localize("duplicated", "Duplicated in SELECT.");
                if (fd_clause.block_contains()) {
                    if (selectClauses.some(x => x.block_contains())) {
                        this.helper.mark(fd_clause, dup_str);
                    }
                }
            ...
            this.helper.mark(ctx, localize("no.select", "There is no SELECT clause for the file."));
        }
        this.visitChildren(ctx);
    }
        ...
        if (reportGroupDescriptionCtx) {
            ...
            if (reportRecordSymbol instanceof ReportRecordSymbol) {
                ...
                if (reportRecordSymbol.isGroup) {
                    ...
                    if (bannedCtx) {
                        this.helper.mark(bannedCtx, localize("for.elementary.only", "Can appear only in elementary data items."));
                    }
                }
                ...
                if (repColumn && !reportRecordSymbol.hasLineNumber) {
                    ...
                    if (!foundLineNum) {
                        this.helper.mark(repColumn, localize("must.have.line.num", "Must have a corresponding LINE NUMBER clause."));
                    }
                }
                ...
                if (sumOrValCtx) {
                    ...
                    if (valCtx && ctx.parent instanceof Report_group_data_description_entryContext) {
                        ...
                        if (!foundColumn) {
                            this.helper.mark(valCtx, localize("must.have.column", "Must also have a COLUMN NUMBER clause."));
                        }
                    }
                }
    ...
    visitScreen_description_entry(ctx: Screen_description_entryContext) {
        ...
        if (screenSymbol instanceof ScreenRecordSymbol) {
            if (screenSymbol.level === 1 && !screenSymbol.name) {
                this.helper.mark(ctx, localize("must.have.screen.name", "Each level 01 item must have a screen name."));
            }
        }
        this.visitChildren(ctx);
    }
        ...
        if (screenDescriptionCtx) {
            ...
            if (screenSymbol instanceof ScreenRecordSymbol) {
                if (screenSymbol.screenType === EScreenType.group) {
                    ...
                    if (blank && blank.LINE()) {
                        this.helper.mark(ctx, localize("only.blank.screen", "Only BLANK SCREEN is allowed in the group."));
                    }
                }
                ...
                if (bannedInVal && screenSymbol.screenType === EScreenType.value) {
                    this.helper.mark(bannedInVal, localize("does.not.allowed.here", "Is not allowed here."));
                }
                ...
                if (bannedInGroup && screenSymbol.screenType === EScreenType.group) {
                    this.helper.mark(bannedInGroup, localize("does.not.allowed.here", "Is not allowed here."));
                }
            }
        }
        this.visitChildren(ctx);
    ...
    visitUsing(ctx: UsingContext) {
        let programCtx = firstContainingContext(ctx, ProgramContext);
        if (!programCtx) {
            this.helper.mark(ctx, localize("must.be.in.program", "Must be inside a program."));
            return;
        }
        let programSymbol = this.helper.symbolTable.symbolWithContext(programCtx);
        if (!(programSymbol instanceof ProgramSymbol)) {
            this.helper.mark(ctx, localize("no.program.symbol", "Program symbol not found."));
            return;
        }
        if (!programSymbol.programDefinition) {
            this.helper.mark(ctx, localize("no.program.definition", "Definition in programSymbol not found."));
            return;
        }
    ...
    visitProg_name(ctx: Prog_nameContext) {
        ...
        if (symbol) {
            if (ctx.identifier_result()) {
                ...
                if (symbol instanceof ProgramSymbol) {
                    this.helper.mark(ctx, localize("must.be.literal", "Must be enclosed in quotes."));
                }
            }
        }
    }
        ...
        if (dataRecordSymbol instanceof DataRecordSymbol) {
                ...
                if (dataRecordSymbol.level === 66) {
                    if (dataRecordSymbol.parent instanceof DataRecordSymbol) {
                        if (dataRecordSymbol.parent.level === 66 || dataRecordSymbol.parent.level === 88) {
                            this.helper.mark(ctx, localize("conditional.cannot.assosiated.66.88", "A CONDITION-NAME cannot be associated with a level 66 or level 88 item."));
                        }
                ...
                switch(dataRecordSymbol.usage) {
                    ...
                    default:
                        if (!dataRecordSymbol.picture) {
                            this.helper.mark(ctx, localize("picture.must.be", "A PICTURE clause is expected."));
                        }
                }
            }
            ...
            if (fileSymbol !== undefined) {
                if (fileSymbol.fileFormat === EFileFormat.Report) {
                    this.helper.mark(ctx, localize("no.records.allowed.in.report", "Description of a report file cannot contain data record entries."));
                }
            }
        }
        this.visitChildren(ctx);
            ...
            if (dataRecordSymbol instanceof DataRecordSymbol) {
                if (ctx.is_external()) {
                    ...
                    if (workingStorageSection) {
                        switch (dataRecordSymbol.level) {
                            ...
                            case 77:
                                break;
                            default:
                                this.helper.mark(ctx, localize("external.invalid.ws", "Only levels 01 and 77 can have the EXTERNAL clause."));
                                break;
                        }
                    } else {
                        this.helper.mark(ctx, localize("external.invalid", "The EXTERNAL clause may appear only in the WORKING-STORAGE section"));
                    }
                    // check redefines
                    ...
                    if (!dataRecordCtx.data_name()) {
                        this.helper.mark(ctx, localize("external.dataname", "A DATA-NAME is expected."));
                    }
                }
                if (ctx.is_global()) {
                    // check level
                    ...
                    if (workingStorageSection) {
                        switch (dataRecordSymbol.level) {
                            ...
                            case 77:
                                break;
                            default:
                                this.helper.mark(ctx, localize("global.invalid.ws", "The GLOBAL clause in WORKING-STORAGE SECTION can only be specified in level 1 or 77."));
                                break;
                        }
                    } else {
                        let fileSection = firstContainingContext(ctx, File_sectionContext);
                        if (fileSection && dataRecordSymbol.level !== 1) {
                            this.helper.mark(ctx, localize("global.invalid.fs", "The GLOBAL clause in FILE SECTION can only be specified in level 1."));
                        } else {
                            this.helper.mark(ctx, localize("global.invalid", "The GLOBAL clause can only be specified in the WORKING-STORAGE or FILE sections."));
                        }
                    }
                    // check name exists
                    if (!dataRecordCtx.data_name()) {
                        this.helper.mark(ctx, localize("global.dataname", "A DATA-NAME is expected."));
                    }
                }
                // test conditional
                if (dataRecordSymbol.level === 88 && !ctx.value_is()) {
                    this.helper.mark(ctx, localize("invalid.conditional", "Must not be in a condition."));
                }
    ...
    visitRenames(ctx: RenamesContext) {
        ...
        if (dataRecordCtx) {
            ...
            if (dataRecordSymbol instanceof DataRecordSymbol) {
                if (dataRecordSymbol.level !== 66) {
                    this.helper.mark(ctx, localize("renames.invalid", "The RENAMES clause can be specified in level 66 only."));
                }
                if (dataRecordSymbol.picture) {
                    this.helper.mark(ctx, localize("no.picture.must.be", "There must be no PICTURE clause."));
                }
        ...
        if (dataRecordCtx) {
            ...
            if (dataRecordSymbol instanceof DataRecordSymbol) {
                switch(dataRecordSymbol.level) {
                    ...
                    case 77:
                    case 88:
                        this.helper.mark(ctx, localize("occurs.invalid", "The OCCURS clause cannot be used in a data description entry in levels 01, 66, 77 or 88."));
                        break;
                }
            }
        }
    ...
    public analizeCallStatement(ctx: Call_statementContext) {
        ...
        if (nameIdentifier) {
            ...
            if (symbol instanceof ProgramSymbol) {
                this.helper.mark(nameIdentifier, localize("must.be.literal", "Must be enclosed in quotes."));
            }
                    ...
                    if (callUsingCtx) {
                        ...
                        if (progSymbol.programDefinition.arguments && progSymbol.programDefinition.arguments.length > 0) {
                            if (progSymbol.programDefinition.arguments.length !== types.length) {
                                this.helper.mark(ctx, localize("invalid.arg.count", "Invalid number of arguments."));
                            } else {
        ...
        if (identifierSymbol instanceof DataRecordSymbol) {
            ...
            if (subscriptingCtx) {
                ...
                if (identifierSymbol.arrayLvl === undefined) {
                    this.helper.mark(subscriptingCtx, localize("not.an.array", "An array is expected."));
                } else {
                    if (identifierSymbol.arrayLvl !== subscriptingCtx.arithmetic_expression().length) {
                        this.helper.mark(subscriptingCtx, localize("doesnt.correspond.array", "Index does not match the array's depth."));
                    }
                }
            ...
            if (referenceCtx) {
                if (returnType !== EDataUsage.DISPLAY ) {
                    this.helper.mark(referenceCtx, localize("not.a.string", "A string is expected."));
                }
