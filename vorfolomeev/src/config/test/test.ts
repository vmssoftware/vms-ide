...
export async function Test()  {
    ...
    setTimeout(async () => {
        ...
        _log_('userpass_get === userpass: ' + (userpass_get === userpass));
        let filetr_get = await cfg.get(FilterSection._section);
        _log_('filetr_get === filter: ' + (filetr_get === filter));
        let collection_get = await cfg.get(HostCollection._section);
        _log_('collection_get === collection: ' + (collection_get === collection));

        userpass.password = "password";
        userpass.username += "+";

        let save_result = await cfg.save();
        _log_('after save ' + save_result);
